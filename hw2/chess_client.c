...
void usage() {
  ...
  printf(
      "If you want to invite other player, input: 3 Your_name Other's_name\n");
  printf("If you want to use chatting room function, input: "
         ":sth_you_want_to_say\n");
...
void thread_server_to_client(void *ptr) {
  ...
  while (1) {
    ...
    case 8: {
      // int board[9];
      char msg[100];
      sscanf(recvbuf, "%d %d%d%d%d%d%d%d%d%d %s", &instruction, &board[0],
